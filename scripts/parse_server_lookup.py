...
def parse_log(infile):
    ...
    with open(infile) as f:
        for line in f.readlines():
            if not "start" in line and not "validationFuture" in line:
                continue
            s = line.split()
            if s[4] != "start" and s[4] != "validationFuture":
                continue
            start = s[4]=="start"
            ...
            loglist.append((ts, fn, start, topic))

    fn_times = []
    fn_start = {} 
    for (ts, fn, start, topic) in loglist:
        ...
        if start:
            ...
            fn_times.append([fn, exec_time, fn_start[fn][topic][idx]])
...
def main(indir, outdir):
    ...
    plt.savefig(join(outdir, "fn_exec_times_lookup.png"), bbox_inches="tight")
